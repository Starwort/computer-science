...
def index(folder: str, footer: str, from_folder: str = "") -> None:
    ...
    with open(join(folder, "index.md"), "wb") as file:
        ...
        file.write(
            nodes_to_tree(collect_nodes(folder, ignore_files)).encode(
                "UTF-8"
            )
        )
...
def rindex(
    folders: Folder, footer: str, previous: str = "", leave: int = 0
) -> None:
...
def run_index(version: str) -> None:
    ...
    """
    page_footer = (
        f"Generated by AutoDocs.index version {version} ⓒ Starwort, 2020"
    )
